...
לבסוף, כיוונו את תדר המחולל ל $70 [Hz]$, כך שלא מתקבל גל עומד. לאחר מכן הקטנו את אורך המיתר עד קבלת גל עומד עם שני מקטעים, האורך  שהתקבל הוא $L=0.98[m]$.
...
\subsection*{תוצאות וניתוח}
תוצאות מדידת התדירויות עבורן התקבל גל עומד בעל ארבע מקטעים מתוארים בטבלה הבאה:
...
\begin{figure}[H]
    ...
    \caption{קו מגמה עבור תדירויות המחולל כפונקציה של המסה התלויה}
\end{figure}
...
\section*{שאלה בונוס}
התבקשנו להוכיח את נוסחא (3) ברקע התיאורטי: $L=n\frac{\lambda}{2}$

יהי שני גלים בעלי  אמפליטודה זהה מתקדמים לכיוונים   שונים, נחשב את הסופרפוזיציה שלהם:
\begin{equation*}
    y(x,t) = Asin(kx-wt) + Asin(kx+wt) = 2Asin(kx)cos(wt)
\end{equation*}
נדרוש שבקצוות יתקיים $y(0,t)=0$ ו- $y(L,t)=0$.
\begin{center}
    \begin{equation*}
        \begin{aligned}
            y(0,t) = 2A\sin(0)\cos(wt) = 0 \\
            y(L,t) = 2A\sin(kL)\cos(wt) = 0 \Rightarrow \sin(kL) = 0 \Rightarrow kL = n\pi, n \in \mathbb{Z}
        \end{aligned}
    \end{equation*}
\end{center}
עבור $k=\frac{2\pi}{\lambda}$, נקבל:

\begin{center}
    \begin{equation*}
        \begin{aligned}
            kL = n\pi \Rightarrow \frac{2\pi}{\lambda}L = n\pi \Rightarrow L = \frac{n\lambda}{2} \quad \Box
        \end{aligned}
    \end{equation*}
\end{center}
